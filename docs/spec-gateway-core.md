...
- 反向代理：单个上游 URL、额外出站头、整包或 SSE 逐帧。
- 健康探测。
...
- **反向代理：** 领域外指定一个 URL 和出站附加头，本领域完成转发（整包或 SSE）。
- **健康探测：** 进程存活可被探测。
...
- 一次 AI 调用的**顺序编排在 `orchestration`**：调 access 额度、调 ai 协议、调 core 转发。core 只被调，不调另外两个。
